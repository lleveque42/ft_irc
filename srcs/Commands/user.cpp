...
/*   Updated: 2022/11/21 19:58:55 by lleveque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	Server::_user(User *user, std::string args) {
	...
	if (!user->getFirstTry())
		return _sendError(user, ERR_ALREADYREGISTERED("USER"));
	for (int i = 0; i < 4; i++) {
		begin = args.begin();
		if (i < 3) {
			end_pos = args.find(' ');
			...
			if (end_pos == args.npos)
				return _sendError(user, ERR_NEEDMOREPARAMS("USER"));
			argsVec.push_back(std::string(begin, end));
			args.erase(begin, end + 1);
		}
		else {
			if (*begin != ':')
				return _sendError(user, ERR_NOPREFIX("USER"));
			begin++;
